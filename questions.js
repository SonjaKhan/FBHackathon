$(window).load(function() {
  ...
  function displayNextQuestion(data) {
    ...
    $("#answers ul li label").each(function(index) {
      $(this).html($("<img>").attr("src", "https://graph.facebook.com/" + question.answers.uids[index] + "/picture?width=100&height=100").attr("alt", "Profile picture of " + question.answers.names[index]));
      $(this).append($("<input>").attr("type", "radio").attr("name", "selection").attr("value", index));
      $(this).append($("<span>").html(question.answers.names[index]));
    });
